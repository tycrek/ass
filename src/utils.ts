...
module.exports = {
	log: console.log,
	path: (...paths: string[]): string => Path.join(__dirname, ...paths),
	saveData: (data: object) => fs.writeJsonSync(Path.join(__dirname, '..', 'data.json'), data, { spaces: 4 }),
	verify: (req, tokens: string[]): boolean => req.headers.authorization && tokens.includes(req.headers.authorization),
	generateToken: (): string => token(),
	generateId: (mode: string, lenth: number, originalName: string): string => // todo: TS interface for mode
	...
	randomHexColour: (): string => { // From: https://www.geeksforgeeks.org/javascript-generate-random-hex-codes-color/
